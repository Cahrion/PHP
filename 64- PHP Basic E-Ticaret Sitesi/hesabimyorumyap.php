...
if(isset($_SESSION["Kullanici"])){
	if(isset($_GET["UrunID"])){
		$GelenUrunID	=	GuvenlikFiltresi($_GET["UrunID"]);
	}else{
		$GelenUrunID	=	"";
	}
	
	if($GelenUrunID!=""){
		$YorumHakSorgusu	=	$VeritabaniBaglantisi->prepare("SELECT * FROM siparisler WHERE UyeId = ? AND UrunId = ?");
		$YorumHakSorgusu->execute([$KullaniciID, $GelenUrunID]);
		$YorumHakKontrol	=	$YorumHakSorgusu->rowCount();
		if($YorumHakKontrol > 0){
?>
<table width="1065" align="center" border="0" cellpadding="0" cellspacing="0">
	<tr>
		...
		<td width="545" valign="top"><table width="545" align="center" border="0" cellpadding="0" cellspacing="0">
			...
			<tr height="30">
				<td valign="top" style="border-bottom: 1px dashed #CCCCCC;">AstroGorya Reklamları.</td>
			</tr>
			<tr height="5">
