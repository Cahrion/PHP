...
if(isset($_SESSION["Kullanici"])){
	if(isset($_POST["OdemeTuruSecimi"])){
		$GelenOdemeTuruSecimi		=	GuvenlikFiltresi($_POST["OdemeTuruSecimi"]);
	}else{
		$GelenOdemeTuruSecimi		=	"";
	}
	if(isset($_POST["TaksitSecimi"])){
		$GelenTaksitSecimi		=	GuvenlikFiltresi($_POST["TaksitSecimi"]);
	}else{
		$GelenTaksitSecimi		=	"";
	}
	...
	if($GelenOdemeTuruSecimi!=""){
		if($GelenOdemeTuruSecimi=="Banka Havalesi"){
			...
			if($SepetSayisi>0){
				...
				$UcretFiyatiIcinSiparislerSorgusu	=	$VeritabaniBaglantisi->prepare("SELECT SUM(ToplamUrunFiyati) AS ToplamUcret FROM siparisler WHERE UyeId = ? AND SiparisNumarasi = ?");
				$UcretFiyatiIcinSiparislerSorgusu->execute([$KullaniciID, $SepetSepetNumarasi]);
				$UcretFiyatiKaydi					=	$UcretFiyatiIcinSiparislerSorgusu->fetch(PDO::FETCH_ASSOC);
					$ToplamUcretimiz	=	$UcretFiyatiKaydi["ToplamUcret"];
